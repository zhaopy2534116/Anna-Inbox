...
class StorageClient(_BaseRpcClient):
    ...
    async def get(
        ...
        try:
            ...
            if "exists" not in result:
                result["exists"] = result.get("value") is not None
            return result
        except StorageError as exc:
            if exc.code == STORAGE_ERR_NOT_FOUND:
                return {"key": key, "value": None, "etag": "", "exists": False}
            raise
    ...
    async def list(
        ...
        *,
        ...
        if kind is not None:
            params["kind"] = kind
        result = await self._call(METHOD_STORAGE_LIST, params, timeout)
        if "items" not in result and "entries" in result:
            result["items"] = result.get("entries") or []
        if "next_cursor" not in result and "cursor" in result:
            result["next_cursor"] = result.get("cursor")
        return result
...
class FilesClient(_BaseRpcClient):
    ...
    async def list(
        ...
        *,
        ...
        if limit is not None:
            params["limit"] = limit
        result = await self._call(self._route(METHOD_FILES_LIST, scope), params, timeout)
        if "items" not in result and "entries" in result:
            result["items"] = result.get("entries") or []
        if "next_cursor" not in result and "cursor" in result:
            result["next_cursor"] = result.get("cursor")
